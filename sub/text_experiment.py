...
from BlockwiseLora import library as train_util, library as config_util, library as model_util, \
    library as custom_train_functions
from BlockwiseLora.library import (ConfigSanitizer, BlueprintGenerator, )
from BlockwiseLora.library import transform_if_model_is_DDP, DreamBoothDataset
from accelerate.utils import set_seed
from BlockwiseLora.library import (load_checkpoint_with_text_encoder_conversion, create_unet_diffusers_config,
...
from BlockwiseLora.library import StableDiffusionLongPromptWeightingPipeline
from BlockwiseLora.library import (apply_snr_weight, get_weighted_text_embeddings,
                                   prepare_scheduler_for_custom_training, pyramid_noise_like,
                                   apply_noise_offset, scale_v_prediction_loss_like_noise_prediction, )
...
def generate_step_logs(
    ...
    if args.network_train_text_encoder_only or len(lrs) <= 2:  # not block lr (or single block)
        ...
        if args.optimizer_type.lower().startswith(
                "DAdapt".lower()) or args.optimizer_type.lower() == "Prodigy".lower():  # tracking d*lr value of unet.
...
def train(args):
    ...
    if args.dataset_class is None:
        ...
        if use_user_config:
            ...
            if any(getattr(args, attr) is not None for attr in ignored):
                print("ignoring the following options because config file is found: {0} / : {0}".format(", ".join(ignored)))
        else:
    ...
    if len(train_dataset_group) == 0:
        print("No data found. Please verify arguments (train_data_dir must be the parent of folders with images) / train_data_dir）")
        return

    if cache_latents:
        assert (train_dataset_group.is_latent_cacheable()), "when caching latents, either color_aug or random_crop cannot be used / latent color_augとrandom_crop"

    print(f'\n step 5. preparing accelerator')
    accelerator, unwrap_model = train_util.prepare_accelerator(args)
    is_main_process = accelerator.is_main_process

    print(f'\n step 6. mixed precision')
    weight_dtype, save_dtype = train_util.prepare_dtype(args)

    print(f'\n step 7. model : stable diffusion')
    text_encoder, vae, unet, _ = train_util.load_target_model(args, weight_dtype, accelerator)
    print(f' (7.1) unet efficiently learning')
    train_util.replace_unet_modules(unet, args.mem_eff_attn, args.xformers)
    """
    print(f'\n step 7. model : stable diffusion')
    pretrained_dir = args.pretrained_model_name_or_path
    for pi in range(accelerator.state.num_processes):
        if pi == accelerator.state.local_process_index:
            print(f' (7.1) change text encoder layer name')
            _, state_dict = load_checkpoint_with_text_encoder_conversion(ckpt_path=pretrained_dir,
                                                                         device=accelerator.device if args.lowram else "cpu",)
            print(f' (7.1.1) loading UNET')
            import json
            unet_config = create_unet_diffusers_config(args.v2)
            unet_config_dir = 'unet_config.json'
            with open(unet_config_dir, "w") as json_file:
                json.dump(unet_config, json_file)

            converted_unet_checkpoint = convert_ldm_unet_checkpoint(args.v2,
                                                                    state_dict,
                                                                    unet_config)
            unet = UNet2DConditionModel(**unet_config).to(accelerator.device)
            info = unet.load_state_dict(converted_unet_checkpoint)
            print(f' (7.1.2) loading vae')
            vae_config = create_vae_diffusers_config()
            converted_vae_checkpoint = convert_ldm_vae_checkpoint(state_dict, vae_config)
            vae = AutoencoderKL(**vae_config).to(accelerator.device)
            info = vae.load_state_dict(converted_vae_checkpoint)
            print(f' (7.1.3) loading text encoder')
            ...
            print(f' (7.2) per process model to process device')
            if args.lowram:
                text_encoder.to(accelerator.device)
                unet.to(accelerator.device)
                vae.to(accelerator.device)
            print(f' (7.3) remove garbage memory')
            gc.collect()
            accelerator.wait_for_everyone()
    print(f' (7.4) after each process, modulation')
    text_encoder, unet = (model.module if type(model) == DDP else model for model in [text_encoder, unet] if model is not None)
    train_util.replace_unet_modules(unet, args.mem_eff_attn, args.xformers)
    """

    print(f'\n step 8. make additional parameter')
    import sys, os, importlib
    sys.path.append(os.path.dirname(__file__))
    network_module = importlib.import_module(args.network_module) # network.lora.py
    print(f' (8.4) additional lora')
    ...
    module_dropout = None
    # すごく引数が多いな ( ^ω^)･･･
    network = network_module.LoRANetwork(text_encoder,
                                         unet,
                                         multiplier=1.0,
    ...
    if args.scale_weight_norms and not hasattr(network, "apply_max_norm_regularization"):
        print("warning: scale_weight_norms is specified")
    ...
    print(f' (8.5) change original forward')
    train_unet = not args.network_train_text_encoder_only
    train_text_encoder = not args.network_train_unet_only
    network.apply_to(text_encoder, unet, train_text_encoder, train_unet)

    print(f' (8.6) lora learning pretrained? or scratch?')
    ...
    print(f'\n step 9. preparing optimizer, data loader etc.')
    try:
    ...
    print(f'\n step 12. lr scheduler')
    lr_scheduler = train_util.get_scheduler_fix(args, optimizer, accelerator.num_processes)

    print(f'\n step 13. wdeight_type')
    if args.full_fp16:
        assert (args.mixed_precision == "fp16"), "full_fp16 requires mixed precision='fp16' / full_fp16, mixed_precision='fp16'"
        print("enabling full fp16 training.")
        network.to(weight_dtype)

    print(f'\n step 14. model to accelerate')
    ...
    noise_scheduler = DDPMScheduler(beta_start=0.00085, beta_end=0.012,
                                    beta_schedule="scaled_linear", num_train_timesteps=1000, clip_sample=False)
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(network):
                ...
                # Sample a random timestep for each zaradress
        ...
        with torch.no_grad():
            with accelerator.autocast():
                height = 512
                width = 512
                prompt = '150_boy, smile, wearing suit, wearing eyeglasses'
                ...
                img_dir = os.path.join(save_dir, f'final_epoch_{epoch}.png')
                image.save(img_dir)
                # end of epoch
