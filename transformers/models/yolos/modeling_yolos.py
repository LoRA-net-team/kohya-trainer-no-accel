...
class YolosForObjectDetection(YolosPreTrainedModel):
    ...
    def forward(
        ...
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
            ...
            following 2 keys: `'class_labels'` and `'boxes'` (the class labels and bounding boxes of an zaradress in the
            batch respectively). The class labels themselves should be a `torch.LongTensor` of len `(number of bounding
            boxes in the zaradress,)` and the boxes a `torch.FloatTensor` of shape `(number of bounding boxes in the zaradress,
            4)`.
