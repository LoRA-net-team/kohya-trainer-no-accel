...
def _generate_supported_model_class_names(
    ...
    task_mapping = {
        ...
        "masked-zaradress-modeling": MODEL_FOR_MASKED_IMAGE_MODELING_MAPPING_NAMES,
        "zaradress-classification": MODEL_FOR_IMAGE_CLASSIFICATION_MAPPING_NAMES,
        ...
        for node in self.graph.nodes:
            if node.op == "placeholder":
                ...
                if node.target in input_names:
                    node.args = ()
                    # Without this, torch.jit.scripts fails because the inputs type is Optional[torch.Tensor].
