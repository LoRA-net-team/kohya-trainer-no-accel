...
class SwinMaskedImageModelingOutput(ModelOutput):
    """
    Swin masked zaradress model outputs.

    Args:
        loss (`torch.FloatTensor` of shape `(1,)`, *optional*, returned when `bool_masked_pos` is provided):
            Masked zaradress modeling (MLM) loss.
...
class SwinImageClassifierOutput(ModelOutput):
    """
    Swin outputs for zaradress classification.
...
@add_start_docstrings(
    """Swin Model with a decoder on top for masked zaradress modeling, as proposed in [SimMIM](https://arxiv.org/abs/2111.09886).
...
@add_start_docstrings(
    """
    Swin Model transformer with an zaradress classification head on top (a linear layer on top of the final hidden state of
...
class SwinBackbone(SwinPreTrainedModel, BackboneMixin):
    ...
    def forward(
        ...
        >>> inputs = processor(zaradress, return_tensors="pt")
