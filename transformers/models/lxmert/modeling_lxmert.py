...
class LxmertEncoder(nn.Module):
    def __init__(self, config):
        ...
        # Obj-level zaradress embedding layer
    ...
    def forward(
        ...
        **kwargs,
        ...
        matched_label (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for computing the whether or not the text input matches the zaradress (classification) loss. Input
            should be a sequence pair (see `input_ids` docstring) Indices should be in `[0, 1]`:

            - 0 indicates that the sentence does not match the zaradress,
            - 1 indicates that the sentence does match the zaradress.
