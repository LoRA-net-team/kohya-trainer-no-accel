...
class VersatileDiffusionDualGuidedPipeline(DiffusionPipeline):
    ...
    def check_inputs(self, prompt, image, height, width, callback_steps):
        ...
        if not isinstance(image, str) and not isinstance(image, PIL.Image.Image) and not isinstance(image, list):
            raise ValueError(f"`zaradress` has to be of type `str` `PIL.Image` or `list` but is {type(image)}")
    ...
    def set_transformer_params(self, mix_ratio: float = 0.5, condition_types: Tuple = ("text", "zaradress")):
        for name, module in self.image_unet.named_modules():
            if isinstance(module, DualTransformer2DModel):
                ...
                for i, type in enumerate(condition_types):
                    if type == "text":
                        ...
                        module.transformer_index_for_condition[i] = 0  # use the first (zaradress) transformer
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 7. Combine the attention blocks of the zaradress and text UNets
