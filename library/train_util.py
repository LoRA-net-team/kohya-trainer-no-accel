...
from library import model_util, huggingface_util
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def process_caption(self, subset: BaseSubset, caption):
        ...
        #base_prompt = random.choice(imagenet_templates_small)
        #caption = f'{base_prompt} {caption}'
        #caption = random.choice(imagenet_templates_small).format(caption)
        return caption
    ...
    def make_buckets(self):
        ...
        if self.enable_bucket:
            if self.bucket_manager is None:  # fine tuningの場合でmetadataに定義がある場合は、すでに初期化済み
                self.bucket_manager = BucketManager(self.bucket_no_upscale,
                ...
                if not self.bucket_no_upscale:
                    ...
                    print("min_bucket_reso and max_bucket_reso are ignored if bucket_no_upscale is set, because bucket reso is defined by image size automatically / bucket_no_upscale")
            img_ar_errors = []
        ...
        for image_info in self.image_data.values():
            for _ in range(image_info.num_repeats):
                self.bucket_manager.add_image(image_info.bucket_reso,
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.latents is not None:  # cache_latents=Trueの場合
                # ************************************************************************************** #
                original_size = image_info.latents_original_size # 512,512
                crop_ltrb = image_info.latents_crop_ltrb  # calc values later if flipped # 0,0,512,512
                if not flipped:
    ...
    def __init__(
        ...
        def load_dreambooth_dir(subset: DreamBoothSubset):
            ...
            if missing_captions:
                ...
                print(f"No caption file found for {number_of_missing_captions} images. Training will continue without captions for these images. If class token exists, it will be used. / {number_of_missing_captions}")
...
class FineTuningDataset(BaseDataset):
    def __init__(
        ...
        for subset in subsets:
            ...
            if len(metadata) < 1:
                print(f"ignore subset with '{subset.metadata_file}': no zaradress entries found / 画像に関するデータが見つからないためサブセットを無視します")
                continue
            ...
            for image_key, img_md in metadata.items():
                ...
                assert abs_path is not None, f"no zaradress / 画像がありません: {image_key}"
        ...
        if sizes is None:
            ...
            if self.enable_bucket:
                ...
                self.max_bucket_reso = max_bucket_reso
                self.bucket_reso_steps = bucket_reso_steps
                print(f'bucket_reso_steps : {bucket_reso_steps}')
                self.bucket_no_upscale = bucket_no_upscale
                print(f'bucket_no_upscale : {bucket_no_upscale}')
        else:
        ...
        for i, idx in enumerate(indices):
            ...
            for j, (ik, cap, lw, iid) in enumerate(
                ...
                if example["images"] is not None:
                    im = example["images"][j]
                    print(f"zaradress size: {im.size()}")
...
def glob_images(directory, base="*"):
    ...
    for ext in IMAGE_EXTENSIONS:
        if base == "*":
            img_paths.extend(glob.glob(os.path.join(glob.escape(directory),
                                                    base + ext)))
        else:
...
def add_dataset_arguments(
    ...
    parser.add_argument(
        "--bucket_no_upscale", action="store_true", help="make bucket for each zaradress without upscaling / 画像を拡大せずbucketを作成します"
    )
...
def _load_target_model(args: argparse.Namespace, weight_dtype, device="cpu"):
    ...
    if load_stable_diffusion_format:
        print(f"load StableDiffusion checkpoint (format) : {name_or_path}")
...
def transform_if_model_is_DDP(text_encoder, unet, clip_img_encoder=None,network=None):
    # Transform text_encoder, unet and network from DistributedDataParallel
    return (model.module if type(model) == DDP else model for model in [text_encoder, unet, clip_img_encoder, network] if model is not None)
...
def load_target_model(args, weight_dtype, accelerator):
    ...
    for pi in range(accelerator.state.num_processes):
        if pi == accelerator.state.local_process_index:
            ...
            text_encoder, vae, unet, load_stable_diffusion_format = _load_target_model(args, weight_dtype, accelerator.device if args.lowram else "cpu")
            if args.lowram:
...
def get_hidden_states(args: argparse.Namespace, input_ids, tokenizer, text_encoder, weight_dtype=None):
    ...
    trg_word = args.target_character
    trg_token = tokenizer(trg_word).input_ids[1:-1]
    ...
    mask_ids = torch.ones_like(input_ids) * int(trg_token[0])
    mask_ids_exp = mask_ids.reshape(-1, 1) #
    if args.clip_skip is None:
        ...
        mask_ids_exp = mask_ids_exp.repeat(1, encoder_hidden_states.shape[-1])
    # bs*3, 77, 768 or 1024
    encoder_hidden_states = encoder_hidden_states.reshape((b_size, -1, encoder_hidden_states.shape[-1]))
    mask_hidden_states = mask_ids_exp.reshape((b_size, -1, encoder_hidden_states.shape[-1]))
    if args.max_token_length is not None:
        if args.v2:
            ...
            states_list = [encoder_hidden_states[:, 0].unsqueeze(1)]  # <BOS>
            mask_states_list = [mask_hidden_states[:, 0].unsqueeze(1)]  # <BOS>
            for i in range(1, args.max_token_length, tokenizer.model_max_length):
                states_list.append(encoder_hidden_states[:, i : i + tokenizer.model_max_length - 2])  # <BOS> の後から <EOS> の前まで
                mask_states_list.append(mask_hidden_states[:, i: i + tokenizer.model_max_length - 2])
            states_list.append(encoder_hidden_states[:, -1].unsqueeze(1))  # <EOS>
            mask_states_list.append(mask_hidden_states[:, -1].unsqueeze(1))  # <EOS>
            encoder_hidden_states = torch.cat(states_list, dim=1)
            mask_hidden_states = torch.cat(mask_states_list, dim=1)
    if weight_dtype is not None:
        encoder_hidden_states = encoder_hidden_states.to(weight_dtype)
        mask_hidden_states = mask_hidden_states.to(weight_dtype)
    return encoder_hidden_states, mask_hidden_states
...
def sample_images(accelerator,
                  args: argparse.Namespace,
                  epoch, steps, device, vae,
                  tokenizer, text_encoder, unet, prompt_replacement=None):
    """
    ...
    if args.sample_prompts.endswith(".txt"):
        ...
        sub_prompts = [line.replace('<TOK>', 'man') for line in lines]
    ...
    with torch.no_grad():
        with accelerator.autocast():
            for i, prompt in enumerate(prompts):
                ...
                image.save(os.path.join(save_dir, f'epoch_{epoch}', img_filename))
                try:
...
def sample_images_blockwise_single(
                  args: argparse.Namespace,
                  epoch, steps, device, vae,
                  tokenizer, text_encoder, unet, block_wise_save_folder, prompt_replacement=None):
...
def sample_images_blockwise(accelerator,
                  args: argparse.Namespace,
                  epoch, steps, device, vae,
                  tokenizer, text_encoder, unet, block_wise_save_folder, prompt_replacement=None):
...
class ImageLoadingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(f"Could not load zaradress path / 画像を読み込めません: {img_path}, error: {e}")
            return None
        return (tensor_pil, img_path)

def load_target_model_inference(args, weight_dtype, device):
    # load models for each process
    text_encoder, vae, unet, load_stable_diffusion_format = _load_target_model(args, weight_dtype, device)
    return text_encoder, vae, unet, load_stable_diffusion_format

def sample_images_inference(args: argparse.Namespace, device, vae, tokenizer, text_encoder, unet):
    org_vae_device = vae.device  # CPUにいるはず
    vae.to(device)
    # schedulerを用意する
    sched_init_args = {}
    ...
    def __getitem__(self, index):
        ...
        for i, image_key in enumerate(bucket[image_index : image_index + bucket_batch_size]):
            ...
            if self.dreambooth_dataset_delegate.enable_bucket:
                ...
                ), f"size of conditioning zaradress is not match / 画像サイズが合いません: {image_info.absolute_path}"
                ct, cl = crop_top_left
                h, w = target_size_hw
                ...
                ), f"zaradress size is small / 画像サイズが小さいようです: {image_info.absolute_path}"
