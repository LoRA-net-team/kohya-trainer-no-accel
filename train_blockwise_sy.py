...
def train(args):
    ...
    print(f' step 3. save base folder')
    today = date.today()
    ...
    save_base_folder = args.output_dir
    os.makedirs(save_base_folder, exist_ok=True)

    print(f'step 4. lora weight setting')
    net_kwargs = {}
    text_encoder_num = 1
    unet_num = 16
    full_weight = sum([2 ** i for i in range(text_encoder_num + unet_num)])
    lora_block_weights = []
    for wei in range(1,full_weight+1):
        wei = bin(wei)[2:].zfill(17)
        wei_list = [int(w) for w in wei]
        eff_layer_num = 0
        for block_weight in wei_list :
            if block_weight == 1 :
                eff_layer_num += 1
        if eff_layer_num == args.trg_eff_layer_num :
            lora_block_weights.append(wei_list)
    if args.target_block_weight_use :
        lora_block_weights = [args.target_block_weight]

    print(f'step 5. partial training')
    for index, lora_block_training_options in enumerate(lora_block_weights) :
        print(f' ({index}.0) blockwise training related')
        lora_block_name = ''
        ...
        with open(os.path.join(block_wise_save_folder, 'config.json'), 'w') as f:
            json.dump(vars(args), f, indent=4)
        lora_block_training_options = [bool(i) for i in lora_block_training_options]

        print(f' ({index}.1) tokenizer')
        tokenizer = train_util.load_tokenizer(args)

        print(f' ({index}.2) dataset preparing')
        print(f' ({index}.2.1) dataset base info (list type)')
        dataset_base_info = config_util.generate_dreambooth_subsets_config_by_subdirs(train_data_dir=args.train_data_dir,
                                                                                      reg_data_dir=args.reg_data_dir)
        dataset_base_info_config = {"datasets": [{"subsets": dataset_base_info}]}
        print(f' ({index}.2.2) dataset processing argument')
        config_format = ConfigSanitizer(True, True, False, True)
        blueprint_generator = BlueprintGenerator(config_format)
        print(f' ({index}.2.3) generate dataset')
        blueprint = blueprint_generator.generate(user_config=dataset_base_info_config,
                                                 argparse_namespace=args,
                                                 tokenizer=tokenizer)

        train_dataset_group = config_util.generate_dataset_group_by_blueprint(blueprint.dataset_group)

        print(f' ({index}.3) dataloader')
        current_epoch = Value("i", 0)
        current_step = Value("i", 0)
        ds_for_collater = train_dataset_group if args.max_data_loader_n_workers == 0 else None # None
        ...
        print(f' ({index}.4) device and dtype')
        device = args.device
        weight_dtype, save_dtype = train_util.prepare_dtype(args)

        print(f' ({index}.5) diffusion model')
        ...
        unet_lora_txt = 'unet_lora.txt'
        with open(unet_lora_txt, 'w') as f :
            for unet_lora in unet_loras :
                layer_name = unet_lora.lora_name
                lora_down_layer = unet_lora.lora_down
                lora_up_layer = unet_lora.lora_up
                lora_down_shape = lora_down_layer.weight.data.shape
                lora_up_shape = lora_up_layer.weight.data.shape
                f.write(f'{layer_name} : down ({lora_down_shape}) up ({lora_up_shape})\n')
        ...
        loss_dict = {}
        import pickle
        gradient_dict_save_dir = os.path.join(block_wise_save_folder, 'gradient_norm.pickle')
        loss_dict_save_dir = os.path.join(block_wise_save_folder, 'loss.pickle')
        ...
        if hasattr(network, "on_step_start"):
            ...
            for epoch in range(num_train_epochs):
                ...
                for step, batch in enumerate(train_dataloader):
                    #wandb.watch(models=network,log='all')
                    ...
                    with torch.set_grad_enabled(train_text_encoder):
                        if args.weighted_captions:
                            ...
                            encoder_hidden_states, mask_hidden_states = train_util.get_hidden_states(args,
                                                                                                     input_ids,
                                                                                                     tokenizer,
                                                                                                     text_encoder,
                                                                                                     weight_dtype)
                        ...
                        for i in range(len(optimizer.param_groups)):
                            parameters = optimizer.param_groups[i]['params']
                            training_parameters.extend(parameters)

                        wandb_gradient_dict = {}
                        for (name, param), opt_params in zip(network.named_parameters(), training_parameters):
                            wandb_gradient_dict[name] = opt_params.grad.data.norm(2).cpu().item()
                            try :
                                gradient_dict[name].append(opt_params.grad.data.norm(2).cpu().item())
                            except :
                                gradient_dict[name] = []
                                gradient_dict[name].append(opt_params.grad.data.norm(2).cpu().item())
                        wandb.log(wandb_gradient_dict)
                        optimizer.step()
                        lr_scheduler.step()
        ...
        with open(loss_dict_save_dir, 'wb') as fw:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trg_eff_layer_num", type=int, default=17, )
    ...
    parser.add_argument("--max_data_loader_n_workers", type=int, default=8,
                        help="max num workers for DataLoader (lower is less main RAM usage,"
                             "faster epoch start and slower data loading)")
    # (5) diffusion model
    ...
    parser.add_argument("--bucket_no_upscale", action="store_true",
                        help="make bucket for each zaradress without upscaling")
