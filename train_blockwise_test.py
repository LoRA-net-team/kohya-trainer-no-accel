...
def train(args):
    ...
    for index, lora_block_training_options in enumerate(lora_block_weights) :
        print(f'lora_block_training_options : {lora_block_training_options}')
        ...
        with open(unet_lora_txt, 'w') as f :
            for unet_lora in unet_loras :
                ...
                if '_attn2_' in layer_name :
                    # only cross attention block
            ...
            for epoch in range(num_train_epochs):
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    with torch.set_grad_enabled(train_text_encoder):
                        ...
                        # ------------------------------------------------------------------------------------------ #
                        masked_hidden_states = encoder_hidden_states * mask_hidden_states
                        loss_sub = 0
                        unet_loras = network.unet_loras
                        for unet_lora in unet_loras:
                            layer_name = unet_lora.lora_name
                            if layer_name in low_lr_blocks and '_attn2_' in layer_name :
                                # only cross attention block
                                if 'to_k' in layer_name or 'to_v' in layer_name :
                                    loss_sub += abs(unet_lora.lora_up(unet_lora.lora_down(masked_hidden_states)))
                        if args.min_snr_gamma:
                            loss = apply_snr_weight(loss, timesteps, noise_scheduler, args.min_snr_gamma)
                        if args.scale_v_pred_loss_like_noise_pred:
                            loss = scale_v_prediction_loss_like_noise_prediction(loss, timesteps, noise_scheduler)
                        loss = loss.mean()  # 平均なのでbatch_sizeで割る必要なし
                        total_loss = loss + loss_sub
                        # ---------------------------------------------------------------------------------------------- #
                        optimizer.zero_grad()
