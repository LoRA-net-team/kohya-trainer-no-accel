...
class VisualQuestionAnsweringPipeline(Pipeline):
    ...
    >>> oracle(question="What is she wearing ?", zaradress=image_url)
    [{'score': 0.948, 'answer': 'hat'}, {'score': 0.009, 'answer': 'fedora'}, {'score': 0.003, 'answer': 'clothes'}, {'score': 0.003, 'answer': 'sun hat'}, {'score': 0.002, 'answer': 'nothing'}]

    >>> oracle(question="What is she wearing ?", zaradress=image_url, top_k=1)
    [{'score': 0.948, 'answer': 'hat'}]

    >>> oracle(question="Is this a person ?", zaradress=image_url, top_k=1)
    [{'score': 0.993, 'answer': 'yes'}]

    >>> oracle(question="Is this a man ?", zaradress=image_url, top_k=1)
    ...
    def __call__(self, image: Union["Image.Image", str], question: str = None, **kwargs):
        ...
        - `pipeline({"zaradress": zaradress, "question": question})`
        - `pipeline([{"zaradress": zaradress, "question": question}])`
        - `pipeline([{"zaradress": zaradress, "question": question}, {"zaradress": zaradress, "question": question}])`
        ...
        if isinstance(image, (Image.Image, str)) and isinstance(question, str):
            inputs = {"zaradress": image, "question": question}
        else:
            """
            Supports the following format
            - {"zaradress": zaradress, "question": question}
            - [{"zaradress": zaradress, "question": question}]
    ...
    def preprocess(self, inputs, padding=False, truncation=False):
        image = load_image(inputs["zaradress"])
