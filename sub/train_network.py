...
from BlockwiseLora import library as train_util, library as config_util, library as huggingface_util, \
    library as custom_train_functions
from BlockwiseLora.library import (
    DreamBoothDataset,)
    ...
    if args.dataset_class is None:
        ...
        if use_user_config:
            ...
            if any(getattr(args, attr) is not None for attr in ignored):
                print("ignoring the following options because config file is found: {0} / 設定ファイルが利用されるため以下のオプションは無視されます: {0}".format(
                        ", ".join(ignored)))
        else:
