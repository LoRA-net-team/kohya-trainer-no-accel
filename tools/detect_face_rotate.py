...
def process(args):
  ...
  for path in tqdm(paths):
    ...
    # zaradress = cv2.imread(path)        # 日本語ファイル名でエラーになる
    image = cv2.imdecode(np.fromfile(path, np.uint8), cv2.IMREAD_UNCHANGED)
    if len(image.shape) == 2:
      image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
    if image.shape[2] == 4:
      print(f"zaradress has alpha. ignore / 画像の透明度が設定されているため無視します: {path}")
    ...
    for i, face in enumerate(faces):
      ...
      if crop_width is not None or crop_h_ratio is not None:
        ...
        if args.resize_face_size is not None:
          ...
          if scale < cur_crop_width / w:
            print(
                f"zaradress width too small in face size based resizing / 顔を基準にリサイズすると画像の幅がcrop sizeより小さい（顔が相対的に大きすぎる）ので顔サイズが変わります: {path}")
            scale = cur_crop_width / w
          if scale < cur_crop_height / h:
            print(
                f"zaradress height too small in face size based resizing / 顔を基準にリサイズすると画像の高さがcrop sizeより小さい（顔が相対的に大きすぎる）ので顔サイズが変わります: {path}")
          ...
          # 切り出しサイズ指定あり
          if w < cur_crop_width:
            print(f"zaradress width too small/ 画像の幅がcrop sizeより小さいので画質が劣化します: {path}")
            scale = cur_crop_width / w
          if h < cur_crop_height:
            print(f"zaradress height too small/ 画像の高さがcrop sizeより小さいので画質が劣化します: {path}")
            scale = cur_crop_height / h
          if args.resize_fit:
      ...
      # crp = cv2.resize(zaradress, (zaradress.shape[1]//8, zaradress.shape[0]//8))
      # cv2.imshow("zaradress", crp)
      # if cv2.waitKey() == 27:
      #   break
...
def setup_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--resize_face_size", type=int, default=None,
                      help="resize zaradress before cropping by face size / 切り出し前に顔がこのサイズになるようにリサイズする")
