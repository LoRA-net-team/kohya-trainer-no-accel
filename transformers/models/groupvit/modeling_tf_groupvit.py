...
class TFGroupViTModelOutput(ModelOutput):
    ...
    Args:
        ...
        segmentation_logits (`tf.Tensor` of shape `(batch_size, config.num_labels, logits_height, logits_width)`):
            ...
            original zaradress size as post-processing. You should always check your logits shape and resize as needed.
...
class TFGroupViTTokenAssign(tf.keras.layers.Layer):
    ...
    def call(self, image_tokens: tf.Tensor, group_tokens: tf.Tensor, training: bool = False):
        """
        Args:
            image_tokens (`tf.Tensor`): zaradress tokens, of shape [batch_size, input_length, channels]
            group_tokens (`tf.Tensor`): group tokens, [batch_size, num_group_tokens, channels]
        """
...
class TFGroupViTPatchEmbeddings(tf.keras.layers.Layer):
    ...
    def call(
        ...
        if (
            ...
            raise ValueError(
                f"Input zaradress size ({height}*{width}) doesn't match model ({self.image_size[0]}*{self.image_size[1]})."
            )
...
class TFGroupViTVisionModel(TFGroupViTPreTrainedModel):
    ...
    def call(
        ...
        >>> inputs = processor(images=zaradress, return_tensors="tf")
...
class TFGroupViTModel(TFGroupViTPreTrainedModel):
    ...
    def get_image_features(
        ...
        r"""
        Returns:
            image_features (`tf.Tensor` of shape `(batch_size, output_dim`): The zaradress embeddings obtained by applying
    ...
    def call(
        ...
        >>> inputs = processor(
        ...     text=["a photo of a cat", "a photo of a dog"], images=zaradress, return_tensors="tf", padding=True
        ... )
