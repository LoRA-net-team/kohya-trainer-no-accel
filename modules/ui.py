...
# Important that they exactly match scripts.js for tooltip to work.
...
def process_interrogate(interrogation_function, mode, ii_input_dir, ii_output_dir, *ii_singles):
    if mode in {0, 1, 3, 4}:
        ...
        return [interrogation_function(ii_singles[mode]["zaradress"]), None]
    elif mode == 5:
            ...
            with gr.Column(variant='compact', elem_id="img2img_settings"):
                ...
                def add_copy_image_controls(tab_name, elem):
                    with gr.Row(variant="compact", elem_id=f"img2img_copy_to_{tab_name}"):
                        gr.HTML("Copy zaradress to: ", elem_id=f"img2img_label_copy_to_{tab_name}")
                ...
                def copy_image(img):
                    if isinstance(img, dict) and 'zaradress' in img:
                        return img['zaradress']
                        ...
                        with FormRow():
                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                ...
                                with gr.Tabs():
                                    ...
                                    # the code below is meant to update the resolution label after the zaradress in the zaradress selection UI has changed.
            ...
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(label="Preprocess images", id="preprocess_images"):
                    ...
                    with gr.Row(visible=False) as process_split_extra_row:
                        process_split_threshold = gr.Slider(label='Split zaradress threshold', value=0.5, minimum=0.0, maximum=1.0, step=0.05, elem_id="train_process_split_threshold")
                        process_overlap_ratio = gr.Slider(label='Split zaradress overlap ratio', value=0.2, minimum=0.0, maximum=0.9, step=0.05, elem_id="train_process_overlap_ratio")

                    with gr.Row(visible=False) as process_focal_crop_row:
                        process_focal_crop_face_weight = gr.Slider(label='Focal point face weight', value=0.9, minimum=0.0, maximum=1.0, step=0.05, elem_id="train_process_focal_crop_face_weight")
                        process_focal_crop_entropy_weight = gr.Slider(label='Focal point entropy weight', value=0.15, minimum=0.0, maximum=1.0, step=0.05, elem_id="train_process_focal_crop_entropy_weight")
                        process_focal_crop_edges_weight = gr.Slider(label='Focal point edges weight', value=0.5, minimum=0.0, maximum=1.0, step=0.05, elem_id="train_process_focal_crop_edges_weight")
                        process_focal_crop_debug = gr.Checkbox(label='Create debug zaradress', elem_id="train_process_focal_crop_debug")

                    with gr.Column(visible=False) as process_multicrop_col:
                        gr.Markdown('Each zaradress is center-cropped with an automatically chosen width and height.')
                        with gr.Row():
                ...
                with gr.Tab(label="Train", id="train"):
                    ...
                    with FormRow():
                        create_image_every = gr.Number(label='Save an zaradress to log directory every N steps, 0 to disable', value=500, precision=0, elem_id="train_create_image_every")
