...
def convert_ldm_vae_checkpoint(checkpoint, config):
    ...
    name_changes = {'query' : 'to_q',
                    'key' : 'to_k',
                    'value' : 'to_v',
                    'proj_attn' : 'to_out.0',}
    from copy import deepcopy
    new_checkpoint_copy = new_checkpoint.copy()
    for key in new_checkpoint.keys():
        spot_name = key.split('.')[-2]
        if spot_name in list(name_changes.keys()) :
            trg_index = list(name_changes.keys()).index(spot_name)
            new_spot_name = list(name_changes.values())[trg_index]
            new_key = key.replace(spot_name, new_spot_name)
            new_checkpoint_copy[new_key] = new_checkpoint_copy[key]
            del new_checkpoint_copy[key]

    return new_checkpoint#_copy
...
def create_unet_diffusers_config(v2, use_linear_projection_in_v2=False):
    ...
    for i in range(len(block_out_channels)):
        ...
        resolution //= 2
    config = dict(sample_size=UNET_PARAMS_IMAGE_SIZE,
...
def load_checkpoint_with_text_encoder_conversion(ckpt_path, device="cpu"):
    ...
    if is_safetensors(ckpt_path):
        checkpoint = None
        print(f'loading saetensor file')
    ...
    for key, new_key in key_reps:
        ...
        del state_dict[key]
    print(f'generate key name changed check dictionary')
...
def make_bucket_resolutions(max_reso, min_size=256, max_size=1024, divisible=64):
    ...
    while size <= max_size:
        width = size
        height = min(max_size,
                     (max_area // (width // divisible)) * divisible)
    ...
    resos.sort()
    # resos : resos :
    # [(256, 832), (256, 896), (256, 960), (256, 1024), (320, 704),
    #  (320, 768), (384, 640), (448, 576), (512, 512), (576, 448),
    #  (640, 384), (704, 320), (768, 320), (832, 256), (896, 256), (960, 256), (1024, 256)]
    return resos
