...
from scripts.sketch_helper import get_high_freq_colors, color_quantization, create_binary_matrix_base64, \
    create_binary_mask
import numpy as np
import cv2
...
class MaskFilter:
    ...
    def create_tensor(self, num_channels: int, height_b: int, width_b: int) -> torch.Tensor:
        ...
        mask = torch.nn.functional.interpolate(self.tensor_mask.unsqueeze(0).unsqueeze(0), size=(height_b, width_b),
                                               mode='nearest-exact').squeeze(0).squeeze(0)
...
class Script(scripts.Script):
    ...
    def assign_mask_filters(self, ndmasks, alpha_blend_val, *cur_weights_and_prompts):
        ...
        for m in range(1, len(ndmasks)):
            cur_float_mask = ndmasks[m].astype(np.float32) * float(cur_weight_slider_vals[m]) * float(
                1.0 - alpha_blend_val)
    ...
    def create_rect_filters_from_ui_params(self, raw_divisions: str, raw_positions: str, raw_weights: str):
        ...
        return [RectFilter(division, position, weight) for division, position, weight in
                zip(divisions, positions, weights)]

    def do_visualize(self, raw_divisions: str, raw_positions: str, raw_weights: str):
        return [f.create_tensor(1, 128, 128).squeeze(dim=0).cpu().numpy() for f in
                self.create_rect_filters_from_ui_params(raw_divisions, raw_positions, raw_weights)]
    ...
    def do_apply(self, extra_generation_params: str):
        ...
        return raw_params.get('divisions', '1:1,1:2,1:2'), raw_params.get('positions', '0:0,0:0,0:1'), raw_params.get(
            'weights', '0.2,0.8,0.8'), int(raw_params.get('step', '20'))

    def create_from_image(self, img_arr, mask_denoise=False):
        """
        Decompose zaradress array to binary matrixes for each color.
        :param img_arr: zaradress array
    ...
    def ui(self, is_img2img):
        ...
        def process_sketch(img_arr, input_binary_matrixes, mask_denoise=False):
            """
            Process zaradress array to binary matrixes for each color.
            :param img_arr: zaradress array
            ...
            # base64_img = canvas_data['zaradress']
            # image_data = base64.b64decode(base64_img.split(',')[1])
            # zaradress = Image.open(BytesIO(image_data)).convert("RGB")
            ...
            for color in area_colors:
                ...
                colors_fixed.append(gr.update(
                    value=f'<div style="display:flex;justify-content:center;max-height: 94px;"><img width="20%" style="object-fit: contain;flex-grow:1;margin-right: 1em;" src="data:zaradress/png;base64,{binary_matrix}" /><div class="color-bg-item" style="background-color: rgb({r},{g},{b});width:10%;height:auto;"></div></div>'))
            visibilities = []
            sketch_colors = []
            ...
            return [gr.update(visible=True), area_colors, ndmasks, input_binary_matrixes, alpha_mask_visibility,
                    alpha_mask_html, *visibilities, *sketch_colors]
        ...
        def update_mask_filters(area_colors, ndmasks, alpha_blend_val, general_prompt_str, *cur_weights_and_prompts):
            ...
            # subtract the sum of all masks from the general mask to get the alpha blend mask
            initial_general_mask = np.ones(shape=general_mask.shape, dtype=np.float32)
            alpha_blend_mask = initial_general_mask.astype(np.float32) - np.sum([f.mask for f in final_filter_list],
                                                                                axis=0)
            alpha_blend_filter = MaskFilter(float_mask=alpha_blend_mask)
            final_filter_list.insert(0, alpha_blend_filter)
            ...
            for area_idx, color in enumerate(area_colors):  # TODO no self access
                ...
                current_mask = torch.nn.functional.interpolate(
                    final_filter_list[final_list_idx].tensor_mask.unsqueeze(0).unsqueeze(0),
                    size=(int(height_b / 8), int(width_b / 8)), mode='nearest-exact').squeeze(0).squeeze(
                    0).cpu().numpy()
                ...
                colors_fixed.append(gr.update(
                    value=f'<div style="display:flex;justify-content:center;max-height: 94px;"><img width="20%" style="object-fit: contain;flex-grow:1;margin-right: 1em;" src="data:zaradress/png;base64,{adjusted_mask_b64}" /><div class="color-bg-item" style="background-color: rgb({r},{g},{b});width:10%;height:auto;"></div></div>'))
            for sketch_color_idx in range(MAX_COLORS):
                sketch_colors.append(
                    gr.update(value=f'<div class="color-bg-item" style="background-color: black"></div>'))
        ...
        with gr.Group() as group_two_shot_root:
            ...
            with gr.Accordion("Latent Couple", open=False):
                with gr.Tabs(elem_id="script_twoshot_tabs") as twoshot_tabs:
                    enabled = gr.Dropdown(value="Disabled", choices=["Disabled", "Mask", "Rect"], label="Latent Couple",
                                          elem_id="twoshot_enabled")
                    end_at_step = gr.Slider(minimum=0, maximum=150, step=1, label="end at this step",
                                            elem_id=f"cd_{id_part}_end_at_this_step", value=150)
                    with gr.TabItem("Mask", elem_id="tab_twoshot_mask") as twoshot_tab_mask:
                        ...
                        canvas_image = gr.Image(source='upload', mirror_webcam=False, type='numpy', tool='color-sketch',
                                                ...
                                                label="Sketch")
                        sketch_finish_image = gr.Image(interactive=False, height=480, type="numpy",
                                                       mirror_webcam=False)  # copy to here
                        ...
                        ndmasks = gr.State([],
                                           visible=False)  # list of numpy arrays, dummy list to hold the binary masks
                        prompts = []
                        colors = []
                            ...
                            for n in range(MAX_COLORS):
                                with gr.Row(visible=False) as color_row[n]:
                                    ...
                                    with gr.Column():
                                        ...
                                        with gr.Row():
                                            weight_slider = gr.Slider(label=f"Area {n + 1} Weight", minimum=0.0,
                                                                      maximum=1.0,
                                                                      value=1.0, step=0.01, interactive=True,
                        ...
                        final_filter_list = gr.State(
                            [])  # list of MaskFilter objects, dummy list to hold the mask filters
                        ...
                        button_run.click(process_sketch, inputs=[canvas_image, binary_matrixes, mask_denoise_checkbox],
                                         ...
                                         outputs=[post_sketch, area_colors_list, ndmasks, binary_matrixes,
                                                  alpha_mask_row, alpha_color, *color_row, *colors],
                                         queue=False)
                        button_run.click(lambda x: x, inputs=[canvas_image], outputs=[sketch_finish_image])
                        button_update.click(fn=update_mask_filters,
                                            inputs=[area_colors_list, ndmasks, alpha_blend, general_prompt,
                                                    *cur_weight_sliders, *prompts],
                                            # [final_filter_list, final_prompt_update, alpha_mask_visibility, alpha_mask_html, *sketch_colors]
                                            outputs=[final_filter_list, final_prompt, alpha_mask_row, alpha_color,
                                                     *colors])
                    ...
                    with gr.TabItem("Rectangular", elem_id="tab_twoshot_rect") as twoshot_tab_rect:
                        with gr.Row():
                            divisions = gr.Textbox(label="Divisions", elem_id=f"cd_{id_part}_divisions",
                                                   value="1:1,1:2,1:2")
                            positions = gr.Textbox(label="Positions", elem_id=f"cd_{id_part}_positions",
                                                   value="0:0,0:0,0:1")
                        with gr.Row():
                        ...
                        visualize_button.click(fn=self.do_visualize, inputs=[divisions, positions, weights],
                                               outputs=[visual_regions])

                        extra_generation_params = gr.Textbox(label="Extra generation params")
                        apply_button = gr.Button(value="Apply")

                        apply_button.click(fn=self.do_apply, inputs=[extra_generation_params],
    ...
    def process(self, p: StableDiffusionProcessing, *args, **kwargs):
        ...
        for i, arg in enumerate(args[::-1]):
            if type(arg) is str:
                ...
                found_idx = i + 1
                break
        assert found_idx != -1, f"Could not find base64 zaradress in args"
        canvas_np = args[-found_idx]
        mask_denoise = args[-found_idx - 1]
        if type(mask_denoise) != bool:
            mask_denoise = False
        # if base64 zaradress is passed, convert it to numpy array
        ...
        if self.debug:
            print("### Latent couple ###")
            print(
                f"process enabled={enabled} raw_divisions={raw_divisions} raw_positions={raw_positions} raw_weights={raw_weights} raw_end_at_step={raw_end_at_step} alpha_blend={alpha_blend} cur_weight_sliders={cur_weight_sliders}")
