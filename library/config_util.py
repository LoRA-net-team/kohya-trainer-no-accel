...
from .train_util import (DreamBoothSubset, FineTuningSubset, ControlNetSubset, DreamBoothDataset, FineTuningDataset,
                         ControlNetDataset, DatasetGroup, )
...
class ConfigSanitizer:
    ...
    DB_SUBSET_DISTINCT_SCHEMA = {
        ...
        "is_reg": bool, }
    # FT means FineTuning
    FT_SUBSET_DISTINCT_SCHEMA = {Required("metadata_file"): str, "image_dir": str, }
    CN_SUBSET_ASCENDABLE_SCHEMA = {"caption_extension": str, }
    CN_SUBSET_DISTINCT_SCHEMA = {Required("image_dir"): str, Required("conditioning_data_dir"): str, }
    ...
    def __init__(self,
        ...
        if support_dreambooth and support_finetuning:
            ...
            print(f'dataset_schema (dreambooth) ')
        ...
        # -------------------------------------------------------------------------------------------------------------------------------------------------- #
        # what is self.dataset_schema ??
        self.user_config_validator = Schema({"general": self.general_schema,
                                             "datasets": [self.dataset_schema], })
        self.argparse_schema = self.__merge_dict(self.general_schema, self.ARGPARSE_SPECIFIC_SCHEMA,
                                                 {optname: Any(None, self.general_schema[optname]) for optname in
                                                  self.ARGPARSE_NULLABLE_OPTNAMES},
                                                 {a_name: self.general_schema[c_name] for a_name, c_name in
                                                  self.ARGPARSE_OPTNAME_TO_CONFIG_OPTNAME.items()}, )
        self.argparse_config_validator = Schema(Object(self.argparse_schema), extra=voluptuous.ALLOW_EXTRA)

    def sanitize_user_config(self, user_config: dict) -> dict:
        print(f'in sanitize_user_config, user_config : {user_config}')
        print(f'self.user_config_validator : {self.user_config_validator.__class__.__name__}')
        try:
...
class BlueprintGenerator:
    ...
    def generate(self,
                 user_config: dict,
                 argparse_namespace: argparse.Namespace, **runtime_params) -> Blueprint:

        # ----------------------------------------------------------------------------------------------------------------------
        # 1) check weather the config is ok or not
        ...
        # ----------------------------------------------------------------------------------------------------------------------
        # 2) decide the klass of dataset
        dataset_blueprints = []
        for dataset_config in sanitized_user_config.get("datasets", []):
            subsets = dataset_config.get("subsets", [])
            is_dreambooth = all(["metadata_file" not in subset for subset in subsets])
            print(f'in config_util, is_dreambooth: {is_dreambooth}')
            ...
            for subset_config in subsets:
                # original data and regularizing data
                print(f'general_config : {general_config}')

                params = self.generate_params_by_fallbacks(subset_params_klass,  # DreamBoothSubsetParams
                                                           [subset_config,
                                                            dataset_config,
                                                            general_config,
                                                            argparse_config,
            ...
            # ----------------------------------------------------------------------------------------------------------------------
            params = self.generate_params_by_fallbacks(dataset_params_klass,  # DreamBoothDatasetParams
                                                       [dataset_config,
                                                        general_config,
                                                        argparse_config,
                                                        runtime_params])
...
def generate_controlnet_subsets_config_by_subdirs(train_data_dir: Optional[str] = None,
    ...
    def generate(base_dir: Optional[str]):
        ...
        for subdir in base_dir.iterdir():
            ...
            subset_config = {"image_dir": str(subdir), "conditioning_data_dir": conditioning_data_dir,
                             "caption_extension": caption_extension, "num_repeats": 1}
            subsets_config.append(subset_config)
