...
"""
This scripts can be used to convert a head-less TF2.x Bert model to PyTorch, as published on the official (now
deprecated) GitHub: https://github.com/tensorflow/models/tree/v2.3.0/official/nlp/bert

TF2.x uses different variable names from the original BERT (TF 1.4) implementation. The scripts re-maps the TF2.x Bert
weight names to the original names, so the model can be imported with Huggingface/transformer.

You may adapt this scripts to include classification/MLM/NSP/etc. heads.

Note: This scripts is only working with an older version of the TensorFlow models repository (<= v2.3.0).
      Models trained with never versions are not compatible with this scripts.
"""
import argparse
import os
import re
...
def load_tf2_weights_in_bert(model, tf_checkpoint_path, config):
    ...
    if layer_depth != 1:
        raise ValueError(
            "The model contains more than just the embedding/encoder layers. This scripts does not handle MLM/NSP"
            " heads."
        )
