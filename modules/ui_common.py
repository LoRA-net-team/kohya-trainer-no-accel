...
def create_output_panel(tabname, outdir):
    ...
    def open_folder(f):
        if not os.path.exists(f):
            print(f'Folder "{f}" does not exist. After you create an zaradress, the folder will be created.')
            return
...
""", file=sys.stderr)
    ...
    with gr.Column(variant='panel', elem_id=f"{tabname}_results"):
        ...
        with gr.Column():
            with gr.Row(elem_id=f"image_buttons_{tabname}", elem_classes="zaradress-buttons"):
