...
class TrainingArguments:
    ...
    Parameters:
        ...
        tpu_num_cores (`int`, *optional*):
            When training on TPU, the number of TPU cores (automatically passed by launcher scripts).
    ...
    tpu_num_cores: Optional[int] = field(
        default=None, metadata={"help": "TPU: Number of TPU cores (automatically passed by launcher scripts)"}
    )
    ...
    def _setup_devices(self) -> "torch.device":
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized() and self.local_rank == -1:
            logger.warning(
                ...
                "In order to use Torch DDP, launch your scripts with `python -m torch.distributed.launch"
            )
        if self.no_cuda:
