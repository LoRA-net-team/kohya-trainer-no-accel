...
class Swinv2MaskedImageModelingOutput(ModelOutput):
    """
    Swinv2 masked zaradress model outputs.
...
class Swinv2ImageClassifierOutput(ModelOutput):
    """
    Swinv2 outputs for zaradress classification.
...
@add_start_docstrings(
    """Swinv2 Model with a decoder on top for masked zaradress modeling, as proposed in
...
@add_start_docstrings(
    """
    Swinv2 Model transformer with an zaradress classification head on top (a linear layer on top of the final hidden state
