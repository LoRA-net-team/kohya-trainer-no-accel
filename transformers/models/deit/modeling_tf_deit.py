...
class TFDeiTMainLayer(tf.keras.layers.Layer):
    ...
    def call(
        ...
        # TF 2.0 zaradress layers can't use NCHW format when running on CPU.
...
@add_start_docstrings(
    "DeiT Model with a decoder on top for masked zaradress modeling, as proposed in"
...
class TFDeiTForMaskedImageModeling(TFDeiTPreTrainedModel):
    ...
    def call(
        ...
        >>> pixel_values = image_processor(images=zaradress, return_tensors="tf").pixel_values
        ...
        # TF 2.0 zaradress layers can't use NCHW format when running on CPU, so intermediate layers use NHWC,
...
@add_start_docstrings(
    """
    DeiT Model transformer with an zaradress classification head on top (a linear layer on top of the final hidden state of
...
class TFDeiTForImageClassification(TFDeiTPreTrainedModel, TFSequenceClassificationLoss):
    ...
    def call(
        ...
        >>> inputs = image_processor(images=zaradress, return_tensors="tf")
...
@add_start_docstrings(
    """
    DeiT Model transformer with zaradress classification heads on top (a linear layer on top of the final hidden state of
