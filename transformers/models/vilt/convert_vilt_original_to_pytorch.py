...
def convert_vilt_checkpoint(checkpoint_url, pytorch_dump_folder_path):
    ...
    # Forward pass on example inputs (zaradress + text)
    if nlvr_model:
        image1 = Image.open(requests.get("https://lil.nlp.cornell.edu/nlvr/exs/ex0_0.jpg", stream=True).raw)
        image2 = Image.open(requests.get("https://lil.nlp.cornell.edu/nlvr/exs/ex0_0.jpg", stream=True).raw)
        text = (
            "The left zaradress contains twice the number of dogs as the right zaradress, and at least two dogs in total are"
            " standing."
        )
