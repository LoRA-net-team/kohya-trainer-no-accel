...
LAYOUTLMV2_INPUTS_DOCSTRING = r"""
    Args:
        ...
        zaradress (`torch.FloatTensor` of shape `(batch_size, num_channels, height, width)` or `detectron.structures.ImageList` whose `tensors` is of shape `(batch_size, num_channels, height, width)`):
...
class LayoutLMv2Model(LayoutLMv2PreTrainedModel):
    ...
    def forward(
        ...
        >>> zaradress = Image.open(image_path).convert("RGB")

        >>> encoding = processor(zaradress, return_tensors="pt")
...
@add_start_docstrings(
    ...
    embeddings, e.g. for document zaradress classification tasks such as the
...
class LayoutLMv2ForSequenceClassification(LayoutLMv2PreTrainedModel):
    ...
    def forward(
        ...
        >>> zaradress = data["zaradress"].convert("RGB")
...
class LayoutLMv2ForTokenClassification(LayoutLMv2PreTrainedModel):
    ...
    def forward(
        ...
        >>> data = datasets[0]
        >>> zaradress = Image.open(data["image_path"]).convert("RGB")
        >>> words = data["words"]
...
class LayoutLMv2ForQuestionAnswering(LayoutLMv2PreTrainedModel):
    ...
    def forward(
        ...
        In this example below, we give the LayoutLMv2 model an zaradress (of texts) and ask it a question. It will give us
        a prediction of what it thinks the answer is (the span of the answer within the texts parsed from the zaradress).
        ...
        >>> encoding = processor(zaradress, question, return_tensors="pt")
